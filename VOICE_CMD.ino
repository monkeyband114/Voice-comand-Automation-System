#define WIFI_SSID     "voke"
#define WIFI_PASSWORD "355blmgat"

#define WIT_VC_ACCESS_TOKEN "SNL5VMCXW6XFQE6DMKYGDCIAZOOLO7EP"
...
void getCommandReceiverMACAddress(const String& commandTarget, const String& commandAction) {
 if (commandTarget == "kitchen") {
    if (commandAction == "on") {
      digitalWrite(FAN_PIN, 0);
      dumbdisplay.writeComment("kitchen on");
    } else if (commandAction == "off") {
      digitalWrite(FAN_PIN, 1);
      dumbdisplay.writeComment("kitchen off");
    }
  }

  else if (commandTarget == "breath"){
    if (commandAction == "on") {
      digitalWrite(LOCK_PIN, 0);
      dumbdisplay.writeComment("SECURTY LOCK on");
    } 
  }
 else if (commandTarget == "water" && commandAction == "off") {
      digitalWrite(LOCK_PIN, 1);
      dumbdisplay.writeComment("SECURTY LOCK off");
  }
  else if (commandTarget == "bedroom"){
    if (commandAction == "on") {
      digitalWrite(LIGHT_PIN, 0);
      dumbdisplay.writeComment("bedroom on");
    } else if (commandAction == "off") {
      digitalWrite(LIGHT_PIN, 1);
      dumbdisplay.writeComment("bedroom off");
    }
  }
 else {
  dumbdisplay.writeComment("nothing");
...
bool sendCommand(const String& commandTarget, const String& commandAction) {
  ...
  getCommandReceiverMACAddress(commandTarget, commandAction);
